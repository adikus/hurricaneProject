...
deconstruction can be demostrated on a single node (figure \ref{fig:halo_exchange}).

\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{images/Neighbour_exchange_2.eps}
\caption{Diagram of halo echange on a single node}
...
Three nodes of the cluster are availble to public and therefore only the rest 17 are
...
Each node runs computaion on a domain of size 150x150x90, which means that the simulation
...
can be seen in figure TODO. Commands used to run these tests can be seen below
...
to determine whether using this approach to paralellisation of LES is usable.
...
something more managable. The cluster on which the evaluation was done, has 64GB
of memory per node available and during the evalution just above tenth of that
...
writing to noral HDD might improve the performace. Other options are using SSD
...
or potentially even thounsands nodes. If the compuation proves to scale well
...
with a large performance tradeoff. While it allows to run the simulation on larger
