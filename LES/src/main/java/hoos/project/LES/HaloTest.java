...
public class HaloTest {
	...
	public static void main(String[] args) {
		...
		float [] p_halo = new float[4 * 2 * (ip+jp+6 + 2) * (kp+2)];
		float [] p_north = new float[4 * (kp+2) * (ip+3)];
		float [] p_east = new float[4 * (kp+2) * (jp+3)];
		float [] p_south = new float[4 * (kp+2) * (ip+3)];
		float [] p_west = new float[4 * (kp+2) * (jp+3)];
		float [] p_northwest = new float[4 * (kp+2)];
		float [] p_northeast = new float[4 * (kp+2)];
		float [] p_southwest = new float[4 * (kp+2)];
		float [] p_southeast = new float[4 * (kp+2)];
		...
		hc.deconstruct(p_halo, p_north, p_south, p_west, p_east, p_northwest, p_northeast, p_southwest, p_southeast, 4, ip+3, jp+3, kp+2);
		...
		hc.construct(p_halo, p_north, p_south, p_west, p_east, p_northwest, p_northeast, p_southwest, p_southeast, 4, ip+3, jp+3, kp+2);
